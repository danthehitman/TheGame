...
namespace TheGameApi.Controllers
{
    public class UsersController : ApiController
    {
        ...
        {
            var items = await _itemRepo.All.Include(j => j.Type).Where(i => i.OwnerId.Value == userId).ToListAsync();
        ...
        {
            var junk = await _junkRepo.All.Include(j => j.Type).Where(j => j.OwnerId.Value == userId).ToListAsync();
